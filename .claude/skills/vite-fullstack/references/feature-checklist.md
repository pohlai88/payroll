...
- **Hub headers:** this file is the API hub — add `@feature` / `@layer route` / `@chain` (+ `@surface` when useful). See [file-headers.md](file-headers.md)
...
- Every new/touched feature-owned file: lean `@feature` + `@layer` (or section banner in shared mega-files)
- API hub route: `@chain` listing ui / client / route / service / repo / schema / spine
- Grep check: `rg "@feature <slug>" src tests` should hit the vertical slice
- Full rules: [file-headers.md](file-headers.md)
...
Before claiming complete: hit the Done criteria in `SKILL.md` (registered route, matching `payrollApi` path, DTO parity, nav/route wiring, RBAC on mutations, migration if schema changed, test when persistence/auth involved, file headers per [file-headers.md](file-headers.md)).
