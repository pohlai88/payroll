...
- List: `companyId?` + `search?` query params (match the existing employees client param style).
- Put joins in `repo/`, never in the route handler.
...
**Template:** `routes/employees.ts` → `service/employees.ts` (`listEmployeesForActor`) → `repo/employees.ts`. This was a fat route and is now the shape to copy for tenant-scoped lists.
