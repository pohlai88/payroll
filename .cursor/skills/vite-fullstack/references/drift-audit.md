...
## Suggested commands (readonly first)

```bash
# FE API surface
rg "requestJson|/v1/" src/web/api/client.ts src/web/api/payroll-api.ts

# Route registration
rg "v1\\.route|Routes\\(" src/server/app.ts src/server/routes

# Shell paths
rg "APP_SHELL_ROUTE_PATHS|Route path=" src/web/shell/app-nav.ts src/web/app.tsx

# Twin DTOs
rg "Keep in sync|EmployeeSummary|PayRunWorkspaceView" src/web/api/types.ts src/repo src/server/routes
```
