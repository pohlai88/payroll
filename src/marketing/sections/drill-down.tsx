        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_22rem] lg:gap-16">
          ...
          <div className="grain overflow-hidden rounded-sm border border-rule bg-paper-card">
            <dl>
              {NODE_KINDS.map((kind) => (
                <div
                  className="flex flex-col gap-2 border-rule-hair border-b px-5 py-4 transition-colors last:border-0 hover:bg-paper-deep/50 sm:flex-row sm:items-baseline sm:gap-6"
                  key={kind.id}
                >
                  <dt className="shrink-0">
                    <span
                      className={`rubric inline-block rounded-sm border px-1.5 py-0.5 tracking-tight ${KIND_COLOR[kind.id] ?? "border-rule text-ink-faint"}`}
                    >
                      {kind.label}
                    </span>
                  </dt>
          ...
          <aside className="flex flex-col gap-8">
            ...
            <div className="rounded-sm border border-rule bg-paper p-4">
              ...
              <ul className="mt-3 space-y-2 text-ink-muted text-xs">
